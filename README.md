...
> [!NOTE]
> Windows is not a supported target for editline.

Features
--------

* Emacs-style line editing with rebindable keys
* UTF-8 aware: cursor movement, deletion, word motion, and wrapped-line
  redisplay all operate on whole glyphs, not bytes
* Command history, with save/restore to a file and a runtime-adjustable
  scrollback size
* TAB completion, including a hook to plug in your own completer
* A callback interface (`rl_callback_*`) for use from an event loop
* An [FSF readline][] compatibility layer, so many programs build
  against editline unchanged
* No dependencies: only termios is used, ncurses/termcap not needed
* Small: the shared library is well under 30 kB
...
int         el_hist_size; /* Size of history scrollback buffer, default: 64 */
...
> [!IMPORTANT]
> RedHat/Fedora/CentOS and other `.rpm`-based distributions do not
> consider `/usr/local` as standard path anymore.  So make sure to
